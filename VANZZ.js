...
{ remoteJid: "62858246299954-1613049930@g.us" } : {}) 
},
message: { 
"videoMessage": { 
"title": `VANZZ OFC🐧`,
...
ptod = "6285824629954@s.whatsapp.net"
stod = `${sender}`
dtod = 'ngetes'
uwu = '```'
...
📱 _*Nomor*_ : @${sender.split('@')[0]}`
 Sendbutdocument(from, tescok, "Create By VANZZ OFC", fs.readFileSync('./media/Ryuu.pdf'),{mimetype:Mimetype.pdf, thumbnail:fs.readFileSync('./media/Ryuu.jpg'), filename:`Ryuu.pdf`,pageCount: 999 }, [{buttonId:`${prefix}listmenu`,buttonText:{displayText:'LIST MENU 🌱'},type:1},{buttonId:`${prefix}allmenu`,buttonText:{displayText:'ALL MENU ⛩️'},type:1},{buttonId:`${prefix}owner`,buttonText:{displayText:'OWNER 🌱'},type:1}], {quoted:fvid, contextInfo: { mentionedJid: [ptod,dtod,stod], forwardingScore: 508, isForwarded: true, externalAdReply:{title:`${tampilUcapan} ${pushname}\nScript Create By VANZZ OFC`,mediaType:"2",thumbnail:fakeimage,mediaUrl:`https://youtu.be/yJYW_dtMAHA`}}})
break
case 'listmenu':
if (isBanned) return reply('Kamu Sudah Di banned!')
...
for (let _ of anu100) {
Ryuu.sendMessage(_.jid, bc100, image, {quoted: ftrol, caption: `*「 BROADCAST BOT WHATSAPP 」*\n\n${body.slice(4)}`})
}
...
case 'sc':
case 'script':
ini_text = `YAH NYARI SC YA CARI DI YT SONO`
reply(ini_text)
break
case 'rules':
if (!isUser) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol}) 
...
if (!isUser) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol}) 
if (isBanned) return reply('Kamu Sudah Di banned!')
if (args.length == 0) return reply(`Example: ${prefix}${command} VANZZ OFC`)
reply(mess.wait) 
anu = args.join(" ")
...
if (!isGroupAdmins && !isOwner) return reply(mess.only.admin)
if (!isBotGroupAdmins) return reply(`Jadikan Bot Sebagai Admin Group!`)
if (args.length == 0) return reply(`Penggunaan ${prefix + command} VANZZ SUPPORT`)
...
Ryuu.sendMessage(from, buffer, video, {quoted: ftrol, caption: 'Succes By : © VANZZ OFC'})
break
case 'tiktokwm':
if (!isUser) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol}) 
if (isBanned) return reply('Kamu Sudah Di banned!')
